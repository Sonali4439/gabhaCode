...
class ActivityBuySubcription extends StatefulWidget {
  ChildList? childList;
  ActivityBuySubcription({super.key, required this.childList});
 // const ActivityBuySubcription({Key? key}) : super(key: key);

  @override
  State<ActivityBuySubcription> createState() => _ActivityBuySubcriptionState();
}

class _ActivityBuySubcriptionState extends State<ActivityBuySubcription> {
...
/*  List<String> _board = ['SSC', 'CBSC', 'ICSC', 'IB'];
 ...
 /* void getAllBoard() async {
    ...
    if (response.body?.status?.statusCode == 0) {
      ...
      dropDownBoard = "${widget.childList!.child!.userSubscription!.subscriptions!.academicYear!.grade!.board!.board}";
  ...
  void getAllGradeByBoard(String boardId) async {
    ...
    // dropDownGrade =  "${widget.childList!.child!.userSubscription!.subscriptions!.academicYear!.grade!.grade}";
    //  gradeList!.add(defaultGrade);
